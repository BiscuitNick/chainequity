...
function getContractAddress(networkName: string): string {
  ...
  if (fs.existsSync(deploymentFile)) {
    ...
    console.log(
      `Using contract address from deployment file: ${deployment.contractAddress}`
    );
  ...
  if (process.env.TOKEN_CONTRACT_ADDRESS) {
    console.log(
      `Using contract address from .env: ${process.env.TOKEN_CONTRACT_ADDRESS}`
    );
    return process.env.TOKEN_CONTRACT_ADDRESS;
  }

  throw new Error(
    "Contract address not found. Please provide it as an argument or deploy first."
  );
}
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (
        ...
        errorMessage.includes("Max rate limit reached")
      ) {
