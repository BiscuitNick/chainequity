...
function getNetworkConfig(networkName: string) {
  ...
  if (!config || !config.rpcUrl) {
    throw new Error(`Network ${networkName} not supported or missing configuration`);
  }
...
async function main() {
  ...
  if (balance === 0n) {
    throw new Error('Deployer account has zero balance. Please fund it with test ETH.');
  }
  ...
  // Deploy contract
  console.log('\n🚢 Deploying contract...');
  const token = await ContractFactory.deploy(tokenName, tokenSymbol, deployerAddress);
