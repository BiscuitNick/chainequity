...
export class CapTableService {
    ...
    generateCapTable(blockNumber) {
        ...
        const entries = balances.map((balance) => {
            ...
            const ownershipPercentage = totalSupply > 0 ? (Number(balanceBigInt) / Number(totalSupply)) * 100 : 0;
            return {
     ...
     */
    getSnapshotAtBlock(blockNumber) {
        ...
        const splitEvents = allEvents.filter((event) => event.event_type === 'StockSplit' && event.block_number <= blockNumber);
    ...
    getOwnershipDistribution() {
        ...
        const topHoldersPercentage = totalSupplyBigInt > 0 ? (Number(top10Total) / Number(totalSupplyBigInt)) * 100 : 0;
     ...
     */
    getBalanceChanges(address, fromBlock, toBlock) {
        ...
        const balanceChanges = allEvents
            ...
            if (fromBlock && event.block_number < fromBlock)
                return false;
            if (toBlock && event.block_number > toBlock)
                return false;
            return true;
        })
            .map((event) => {
 ...
 */
export function createCapTableService(db) {
