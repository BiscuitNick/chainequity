...
describe('ChainEquityToken', function () {
  ...
  describe('Allowlist Management', function () {
    describe('approveWallet', function () {
      ...
      it('Should revert if called by non-owner', async function () {
        await expect(token.write.approveWallet([addr2.account.address], { account: addr1.account }))
          .to.be.rejected;
      });

      it('Should revert for zero address', async function () {
        const zeroAddress = '0x0000000000000000000000000000000000000000' as Address;
        await expect(token.write.approveWallet([zeroAddress])).to.be.rejected;
      });
    });
    ...
    describe('revokeWallet', function () {
      ...
      it('Should revert if called by non-owner', async function () {
        await expect(token.write.revokeWallet([addr1.account.address], { account: addr1.account }))
          .to.be.rejected;
      });
    });
  });
  ...
  describe('Minting', function () {
    ...
    it('Should revert if recipient not approved', async function () {
      await expect(token.write.mint([addr2.account.address, MINT_AMOUNT])).to.be.rejected;
    });

    it('Should revert for zero address', async function () {
      const zeroAddress = '0x0000000000000000000000000000000000000000' as Address;
      await expect(token.write.mint([zeroAddress, MINT_AMOUNT])).to.be.rejected;
    });

    it('Should revert for zero amount', async function () {
      await expect(token.write.mint([addr1.account.address, 0n])).to.be.rejected;
    });
  });
  ...
  describe('Stock Splits', function () {
    ...
    it('Should revert if multiplier is zero', async function () {
      await expect(token.write.executeSplit([0n])).to.be.rejected;
    });
  ...
  describe('Symbol and Name Updates', function () {
    ...
    it('Should revert for empty symbol', async function () {
      await expect(token.write.updateSymbol([''])).to.be.rejected;
    });
    ...
    it('Should revert for empty name', async function () {
      await expect(token.write.updateName([''])).to.be.rejected;
    });
  });
