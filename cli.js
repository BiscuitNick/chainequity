...
function getNetworkConfig(network = 'localhost') {
    const configs = {
        localhost: {
...
function createIssuerService(network = 'localhost') {
...
wallet
    ...
    try {
        ...
        console.log(chalk.gray('Address:'), address);
        console.log(chalk.gray('Status:'), isApproved ? chalk.green('✓ Approved') : chalk.red('✗ Not Approved'));
    }
    catch (error) {
...
corporate
    ...
    .option('-n, --network <network>', 'Network to use', 'localhost')
    .action(async (multiplier, options) => {
...
corporate
    ...
    .option('-n, --network <network>', 'Network to use', 'localhost')
    .action(async (newSymbol, options) => {
...
program
    ...
    try {
        ...
        if (options.format === 'csv') {
            ...
            console.log(chalk.gray('Generated At:'), new Date(snapshot.generatedAt).toLocaleString());
            ...
            console.log(chalk.gray('  Save to file: npm run cli snapshot', blockNumber, '--format json -o snapshot.json'));
...
program
    ...
    try {
        ...
        console.log(chalk.gray('Top 10 Concentration:'), distribution.topHoldersPercentage.toFixed(2) + '%');
